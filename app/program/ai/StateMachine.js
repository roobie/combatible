...
(function () {
    ...
    var StateMachine = {
        ...
        create: function(cfg, target) {
            ...
            // allow for a simple string,
            // or an object with { state: 'foo', event: 'setup', defer: true|false }
            var initial = (typeof cfg.initial === 'string') ?
                    { state: cfg.initial } : cfg.initial;
            ...
            var add = function(e) {
                ...
                if (transitions.indexOf(e.name) === -1) {
                    transitions.push(e.name);
                }

                if (typeof from === "string"
                    && states.indexOf(e.from) === -1) {
                    states.push(e.to);
                }

                if (typeof e.to === "string"
                    && states.indexOf(e.to) === -1) {
                    states.push(e.to);
                }
            ...
            fsm.is = function(state) {
                return (state instanceof Array) ?
                    (state.indexOf(this.current) >= 0) :
                    (this.current === state);
            };
            fsm.can = function(event) {
                return !this.transition &&
                    (map[event].hasOwnProperty(this.current) ||
